...
Open `/game/plank.html` directly or choose **plank_pilot.exe** in the WIP lab. Fly a pixel selfie through rocks and purple mystery boxes. Each pilot starts with **three hearts**: a collision costs one heart, while the opposite lane has a **? box**. Collecting one immediately changes your face to a silly pixel filter: frog hat, shades, moustache, googly eyes, clown nose, or crown. The effect remains until the next pickup and never repeats the current look. It decorates only the small avatar and works on the practice face too. The optional live face backdrop stays dim and unfiltered. At zero hearts a solo round ends; in co-op that pilot spectates until their buddy finishes or also runs out of hearts. Pick **30, 45, 60, or 90 seconds**, or enter **Custom seconds** (any whole number from 1 to 3,600). Typing a custom duration replaces the preset selection; choosing a preset clears the custom field. Both solo and buddy flights use the entered duration.

## Pace and camera response

Obstacles now change sides on 80% of random choices and never repeat one side more than twice. Over the first 20 seconds, their arrival spacing ramps from about **1.5 seconds to 1.05 seconds**, and their approach shortens from about **2.7 seconds to 2.1 seconds**. A normal 30-second flight contains 23 obstacles, up from 13. The first obstacle still takes about 3.5 seconds to reach you after the countdown.

Camera mode measures recent usable detections and uses six typical detection intervals to choose the minimum spacing for newly scheduled obstacles, between 1.05 and 2.1 seconds. Slower tracking also gives new obstacles a longer approach. Existing visible obstacles keep their timing, and both buddies always share the same pattern and pace. Stale input freezes the game after 350–750 ms depending on the measured detection rate; missing faces freeze it immediately. Inference time is included in the 15 FPS camera budget, and face steering uses less smoothing for a quicker response. These adaptations are covered by simulated timing tests; physical-phone response still needs checking below. Faster-round records use a new key so old easy-round scores stay separate.
